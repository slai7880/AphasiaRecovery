...
def evaluateSVMParamSearchExperiments():
    ...
    figDir = os.path.join(PATH_TO_FIGURES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "SVMParameterSearch_RSOnly")
    pathlib.Path(figDir).mkdir(parents = True, exist_ok = True)
    for include_RS in include_RS_choices:
        inputDir = os.path.join(PATH_TO_TABLES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "LOO", "SVMParameterSearch_RSOnly", "RS" + include_RS)
        ...
        plt.close(fig)
        '''
        F1ScoresOptimal["RBF"].append(F1Scores.max())
        ...
        plt.close(fig)
        '''
        F1ScoresOptimal["Linear"].append(F1Scores.max())
    fig, ax = plt.subplots(figsize = (28, 10))
    for key in F1ScoresOptimal:
        ax.plot(F1ScoresOptimal[key], label = key)
    ax.set_title("Optimal SVM F1 Scores with Top K RS Features", fontsize = 16)
    ax.set_xlabel("K", fontsize = 14)
    ax.set_xticks([i for i in range(len(include_RS_choices))])
    ax.set_xticklabels(include_RS_choices)
    ax.grid(True)
    ax.legend()
    fig.savefig(os.path.join(figDir, "OptimalF1Scores.png"))
    plt.close(fig)
    
    return F1ScoresOptimal
...
def evaluateRFParamSearchExperiments():
    ...
    figDir = os.path.join(PATH_TO_FIGURES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "RFParameterSearch_RSOnly")
    pathlib.Path(figDir).mkdir(parents = True, exist_ok = True)
    for include_RS in include_RS_choices:
        inputDir = os.path.join(PATH_TO_TABLES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "LOO", "RFParameterSearch_RSOnly", "RS" + include_RS)
        ...
        for i in range(len(criteria)):
            ...
            F1ScoresOptimal[criteria[i]].append(F1Scores[criteria[i]].max())
        '''
    ...
    for key in F1ScoresOptimal:
        ax.plot(F1ScoresOptimal[key], label = key)
    ax.set_title("Optimal RF F1 Scores with Top K RS Features", fontsize = 16)
    ax.set_xlabel("K", fontsize = 14)
    ax.set_xticks([i for i in range(len(include_RS_choices))])
    ax.set_xticklabels(include_RS_choices)
    ax.grid(True)
...
def evaluateAdaBoostParamSearchExperiments():
    ...
    include_RS_choices = [str(i) for i in range(1, 101)]
    F1ScoresOptimal = []
    figDir = os.path.join(PATH_TO_FIGURES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "AdaBoostParameterSearch_RSOnly")
    pathlib.Path(figDir).mkdir(parents = True, exist_ok = True)
    for include_RS in include_RS_choices:
        inputDir = os.path.join(PATH_TO_TABLES, "AllFeaturesExperiments_" + DATA_VERSION, "Transformed", "Classification", "Untuned", "LOO", "AdaBoostParameterSearch_RSOnly", "RS" + include_RS)
        ...
        '''
    fig, ax = plt.subplots(figsize = (28, 10))
    ax.plot(F1ScoresOptimal)
    ax.set_title("Optimal AdaBoost F1 Scores with Top K RS Features", fontsize = 16)
    ax.set_xlabel("K", fontsize = 14)
    ax.set_xticks([i for i in range(len(include_RS_choices))])
    ax.set_xticklabels(include_RS_choices)
    ax.grid(True)
